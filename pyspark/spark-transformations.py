...
r1 = student_df.repartition(2, "ID")
...
filtered_students = student_df \
    .select("ID", "Name", "Address", "Marks", "DOB") \
    .filter("Marks is not NULL") \
    .filter("DOB not like '2014-%'") \
    .filter(student_df["ID"] <= 200) \
    .dropDuplicates() \
    .limit(100)

# Create new column "Name" by concatenating first and last names
students_df = filtered_students.withColumn("Random", concat("Name", lit(" "), "ID"))
...
students_df = students_df.fillna({"ID": "100"})
...
# OrderBy and Show
students_df \
    .orderBy("Age", "ID", ascending=False) \
    .show(10)
...
selected_fields_df = student_df.select("ID", "Name", "Address.city", "Address.state", "Address")
...
exploded_df = student_df.select("ID", "Name", explode("Marks").alias("Grade"))
exploded_df.show()

# Create a single array from an array of arrays
# flattened_df = student_df.select("ID", "Name", flatten("Marks").alias("Grade"))
# flattened_df.show()

# Reduce the "Marks" array column to calculate the sum
reduced_df = exploded_df.groupby("ID").sum("Grade")
reduced_df.show()

# Find the minimum grade in the "Marks" array column; Adding sort just because
min_grade = student_df.select("ID", "Name", "Marks", array_min(array_sort("Marks", reverse=True)))
min_grade.show()

# Find the maximum grade in the "Marks" array column
max_grade = student_df.select("ID", "Name", "Marks", array_max("Marks").alias("Max_Grade"))
max_grade.show()
...
udf_result = student_df.withColumn("UDF_COLUMN", udf_func(col("ID")))
udf_result.show()
